...
export default function MemberProfile() {
  ...
  const fetchMember = async () => {
    ...
    if (data) {
      ...
      const { data: courseData } = await supabase
        .from('courses').select('*, classes(name)')
  ...
  // --- 换装编辑器 ---
  const editorFont = Math.round(CHAR_W * ACC_FONT_RATIO) // 28px
  ...
  const handleAddAccessory = (emoji) => {
    ...
    selIdxRef.current = next.length - 1
  }
  ...
  const handleScale = (delta) => {
    ...
    const next = outfitRef.current.map((item, i) => {
      ...
      return { ...item, s: Math.round(Math.max(0.3, Math.min(3, item.s + delta)) * 10) / 10 }
    })
    outfitRef.current = next
    setOutfit(next)
    saveOutfit(member.name, next)
  ...
  const handleRotate = (delta) => {
    ...
    const next = outfitRef.current.map((item, i) => {
      if (i !== selIdx) return item
      return { ...item, r: Math.round((item.r + delta) % 360) }
    })
    outfitRef.current = next
    setOutfit(next)
    saveOutfit(member.name, next)
  ...
  const handleOpacity = (delta) => {
    if (selIdx == null) return
    const next = outfitRef.current.map((item, i) => {
      if (i !== selIdx) return item
      return { ...item, o: Math.round(Math.max(0.1, Math.min(1, (item.o || 1) + delta)) * 10) / 10 }
    })
    outfitRef.current = next
    setOutfit(next)
    saveOutfit(member.name, next)
  ...
  const handlePointerDownItem = (e, idx) => {
    ...
    setSelIdx(idx); selIdxRef.current = idx; setDragging(true)
  ...
  const handlePointerUpItem = (e) => {
    e.preventDefault()
    const el = dragElRef.current
    if (el) { el.releasePointerCapture(e.pointerId); dragElRef.current = null }
  ...
  if (!member) {
    return <div className="page-content" style={{ textAlign: 'center', padding: 60, color: 'var(--color-text-secondary)' }}>成员不存在</div>
  }
  ...
  return (
    <div className="page-content">
      <button onClick={() => navigate('/')} aria-label="返回首页" style={{
        background: 'none', border: 'none', cursor: 'pointer', fontSize: 14,
        color: 'var(--color-brand-emphasis)', marginBottom: 12, padding: 0,
      ...
      <div className="card" style={{ textAlign: 'center', marginBottom: 16 }}>
        ...
        <div ref={charRef} style={{
          ...
          {avatarSrc ? (
            <img src={avatarSrc} alt={member.name} draggable={false} style={{
              ...
              objectPosition: 'bottom center', position: 'relative', zIndex: 1, pointerEvents: 'none',
            }} />
          ) : (
            <div style={{
          ...
          {outfit.map((item, i) => {
            ...
            return (
              <div key={i}
                onPointerDown={(e) => { if (!showDressUp) return; handlePointerDownItem(e, i) }}
                ...
                style={{
                  ...
                  fontSize: editorFont, zIndex: isSel ? 5 : 2, opacity: o,
        ...
        {/* 换装按钮 */}
        <button onClick={() => { setShowDressUp(!showDressUp); setSelIdx(null); selIdxRef.current = null }}
          style={{
            fontSize: 13, padding: '6px 16px', marginBottom: 10,
            border: '1px solid var(--color-brand-primary)', borderRadius: 16,
            background: showDressUp ? 'var(--color-brand-primary)' : 'transparent',
        ...
        {showDressUp && (
          <div style={{
            background: 'var(--color-surface-primary)', borderRadius: 12, padding: 10, marginBottom: 10,
            ...
            {selItem && (
              <div style={{
                ...
                gap: 4, marginBottom: 10, flexWrap: 'wrap',
                padding: '6px 8px', background: 'var(--color-surface-card)', borderRadius: 10, fontSize: 12,
              }}>
                <span style={{ fontSize: 20 }}>{selItem.e}</span>
                <span style={{ fontSize: 10, color: 'var(--color-text-secondary)' }}>x:{selItem.x}% y:{selItem.y}%</span>

                <button onClick={() => handleScale(-0.1)} style={s.btn} title="缩小"><Minus size={12} /></button>
                <span style={s.val}>{selItem.s}x</span>
                <button onClick={() => handleScale(0.1)} style={s.btn} title="放大"><Plus size={12} /></button>

                <button onClick={() => handleRotate(-15)} style={s.btn} title="左转">↺</button>
                <span style={s.val}>{selItem.r}°</span>
                <button onClick={() => handleRotate(15)} style={s.btn} title="右转">↻</button>

                <span style={{ fontSize: 10, color: 'var(--color-text-secondary)', marginLeft: 4 }}>透明</span>
                <button onClick={() => handleOpacity(-0.1)} style={s.btn} title="更透明"><Minus size={10} /></button>
                <span style={s.val}>{Math.round((selItem.o || 1) * 100)}%</span>
                <button onClick={() => handleOpacity(0.1)} style={s.btn} title="更不透明"><Plus size={10} /></button>

                <button onClick={handleRemoveSelected} style={{ ...s.btn, color: '#E74C3C', borderColor: '#E74C3C', marginLeft: 8 }}>删除</button>
              </div>
            )}

            {selItem == null && (
              <p style={{ fontSize: 11, color: 'var(--color-text-secondary)', marginBottom: 8 }}>
                点击下方配饰添加 → 拖动角色身上的配饰调整位置
              </p>
            )}
            ...
            <div style={{ display: 'flex', gap: 3, marginBottom: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
              {CAT_NAMES.map((cat) => (
                <button key={cat} onClick={() => setActiveCat(cat)} style={{
                  fontSize: 11, padding: '3px 8px', borderRadius: 10, border: 'none',
                  background: activeCat === cat ? 'var(--color-brand-primary)' : 'var(--color-surface-card)',
                  color: activeCat === cat ? '#fff' : 'var(--color-text-secondary)', cursor: 'pointer',
                }}>{cat}</button>
              ))}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4 }}>
              {ACCESSORY_CATS[activeCat].map((emoji) => (
                <button key={emoji} onClick={() => handleAddAccessory(emoji)} style={{
                  aspectRatio: '1', borderRadius: 10,
                  border: '1px solid var(--color-brand-subtle)', background: 'var(--color-surface-card)',
...
const s = {
  btn: {
    ...
    border: '1px solid var(--color-brand-subtle)',
    background: 'var(--color-surface-primary)', cursor: 'pointer',
    display: 'flex', alignItems: 'center', justifyContent: 'center',
